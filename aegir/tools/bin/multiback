...
_verify_boa_keys() {
  if [ "${_tRee}" = "pro" ] || [ "${_tRee}" = "dev" ]; then
    ...
    if [ -e "/var/opt/_encN.${_encName}.tmp" ] && [ -e "/var/opt/_encN_local.${_encName}.tmp" ]; then
      _diffTestIf=$(diff -w -B /var/opt/_encN.${_encName}.tmp /var/opt/_encN_local.${_encName}.tmp)
...
_monthly_cleanup() {
  if [ -e "${_LOGPTH}/${_BUCKET_NAME}.randomize.cleanup.log" ]; then
    _RCL=$(cat ${_LOGPTH}/${_BUCKET_NAME}.randomize.cleanup.log 2>/dev/null | tr -d '\n')
    _RCL=${_RCL//[^1-5]/}
  else
    _RCL=$((RANDOM%5+1))
    _RCL=${_RCL//[^1-5]/}
...
_randomize_full() {
  if [ -e "/root/.randomize_duplicity_full_backup_day.cnf" ]; then
    if [ -e "${_LOGPTH}/${_BUCKET_NAME}.randomize.full.log" ]; then
      _RDW=$(cat ${_LOGPTH}/${_BUCKET_NAME}.randomize.full.log 2>/dev/null | tr -d '\n')
      _RDW=${_RDW//[^1-7]/}
      _MODE="incremental"
    else
      _RDW=$((RANDOM%7+1))
