...
enable_chattr() {
  ...
  if [ ! -z "${isTest}" ] && [ -d "/home/$1" ]; then
    ...
    _U_TP="/home/$1/.tmp"
    _U_II="${_U_HD}/php.ini"
    if [ ! -e "${_U_HD}/.ctrl.400stableQ2.pid" ]; then
    ...
    if [ "${_PHP_CLI_UPDATE}" = "YES" ] \
      || [ ! -e "${_U_II}" ] \
      || [ ! -e "${_U_HD}/.ctrl.400stableQ2.pid" ]; then
      mkdir -p ${_U_HD}
      rm -f ${_U_HD}/.ctrl.php*
      rm -f ${_U_II}
      ...
      if [ -e "${_U_II}" ]; then
        ...
        echo > ${_U_HD}/.ctrl.400stableQ2.pid
      fi
    fi
...
manage_user() {
  for pthParentUsr in `find /data/disk/ -maxdepth 1 -mindepth 1 | sort`; do
    if [ -e "${pthParentUsr}/config/server_master/nginx/vhost.d" ] \
      ...
      if [ ! -e "${dscUsr}/.tmp/.ctrl.400stableQ2.pid" ]; then
        ...
        echo OK > ${dscUsr}/.tmp/.ctrl.400stableQ2.pid
      fi
      if [ ! -e "${dscUsr}/static/control/.ctrl.400stableQ2.pid" ]; then
        ...
        echo OK > ${dscUsr}/static/control/.ctrl.400stableQ2.pid
      fi
      ...
      if [ -e "${pthParentUsr}/clients" ] && [ ! -z ${_USER} ]; then
        ...
        if [ -d "/home/${_USER}.ftp" ]; then
          ...
          if [ ! -e "/home/${_USER}.ftp/.tmp/.ctrl.400stableQ2.pid" ]; then
            ...
            echo OK > /home/${_USER}.ftp/.tmp/.ctrl.400stableQ2.pid
          fi
...
if [ ! -e "/home/.ctrl.400stableQ2.pid" ]; then
  chattr -i /home
  chmod 0711 /home
  ...
  touch /home/.ctrl.400stableQ2.pid
fi
