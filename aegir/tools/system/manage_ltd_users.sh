  ...
  if [ ! -z "$1" ] && [ -d "/home/$1" ] ; then
    ...
    if [ ! -e "$_U_HD/.ctrl.240dev.txt" ] ; then
      if [[ "$_HOST_TEST" =~ ".host8." ]] || [ "$_VMFAMILY" = "VS" ] ; then
        ...
        rm -f $_U_HD/{drupalgeddon,drush_ecl}
        rm -f $_U_HD/usr/{drush_make,registry_rebuild,clean_missing_modules}
        rm -f $_U_HD/usr/{drupalgeddon,drush_ecl}
        rm -f $_U_HD/.ctrl*
    ...
    _CHECK_USE_PHP_CLI=`grep "/opt/php" \
      /data/disk/${_USER}/tools/drush/drush.php`
    _PHP_V="56 55 54 53"
    for e in $_PHP_V; do
...
enable_newrelic() {
  _LOC_NEW_RELIC_KEY=`cat /data/disk/${_USER}/static/control/newrelic.info`
...
switch_php() {
  ...
  if [ -e "/data/disk/${_USER}/static/control/fpm.info" ] \
    ...
    if [ ! -e "/data/disk/${_USER}/static/control/hhvm.info" ] \
      ...
      _LOC_PHP_FPM_VERSION=`cat /data/disk/${_USER}/static/control/fpm.info`
