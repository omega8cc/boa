...
_fix_node_in_lshell_access() {
  ...
  if [ -e "/etc/lshell.conf" ]; then
    ...
    if [[ "${_PrTestPhantom}" =~ "PHANTOM" ]] \
      ...
      _ALLOW_NODE=NO
      sed -i -E "s/, '?(node|npm|npx|find|scp)'?,/,/g" /etc/lshell.conf /var/xdrago/conf/lshell.conf
    fi
  fi
}
...
_fix_php_in_lshell_access() {
  ...
  if [ -e "/etc/lshell.conf" ]; then
    ...
    if [[ "${_PrTestPhantom}" =~ "PHANTOM" ]] \
      ...
      _ALLOW_PHP=NO
      sed -i -E "s#,\s*'(php83|php82|php81|php74|php56|/opt/php83/bin/php|/opt/php82/bin/php|/opt/php81/bin/php|/opt/php74/bin/php|/opt/php56/bin/php)',#,/g" /etc/lshell.conf /var/xdrago/conf/lshell.conf
    fi
  fi
}
