...
disable_chattr () {
  if [ ! -z "$1" ] && [ -d "/home/$1" ] ; then
    ...
    if [ "$1" != "${_THIS_HM_USER}.ftp" ] ; then
      ...
      if [ ! -d "/data/disk/${_THIS_HM_USER}/.drush/usr/drupalgeddon" ] || [ ! -e "/data/disk/${_THIS_HM_USER}/static/control/.drupalgeddon.in.014.pid" ] ; then
        ...
        touch /data/disk/${_THIS_HM_USER}/static/control/.drupalgeddon.in.014.pid
      fi
      if [ ! -L "/home/$1/.drush/usr/drupalgeddon" ] && [ -d "/data/disk/${_THIS_HM_USER}/.drush/usr/drupalgeddon" ] ; then
      ...
      if [ -e "$Plr/modules/o_contrib_seven" ] ; then
        if [ -L "/home/${_THIS_HM_USER}.ftp/.drush/usr/drupalgeddon" ] ; then
          ...
          if [[ "$_DGDD_TEST" =~ "No evidence of known Drupalgeddon exploits found" ]] ; then
            _DO_NOTHING=YES
          elif [[ "$_DGDD_TEST" =~ "has security vulnerabilities" ]] ; then
            _DO_NOTHING=YES
...
update_solr () {
  # $1 is module
  # $2 is solr core path
  if [ ! -z $1 ] && [ ! -e "$2/conf/BOA-2.3.6.conf" ] && [ -e "/var/xdrago/conf/solr" ] && [ -e "$2/conf" ] ; then
...
setup_solr () {
  ...
  if [ -e "$_DIR_CTRL_FILE" ] ; then
    ...
    if [[ "$_SOLR_UPDATE_CONFIG_TEST" =~ "solr_update_config = YES" ]] ; then
      if [ "$_SOLR_CUSTOM_CONFIG_RESULT" = "YES" ] || [ -e "${_SOLR_DIR}/conf/BOA-2.3.6.conf" ] ; then
        _DO_NOTHING=YES
      else
...
add_note_platform_ini () {
  if [ -e "$_CTRL_FILE" ] ; then
    ...
    echo ";;  may not include all options available after upgrade to BOA-2.3.6"         >> $_CTRL_FILE
    echo ";;" >> $_CTRL_FILE
...
if [ ! -e "/var/backups/fix-sites-all-permsissions-2.3.6.txt" ] ; then
  ...
  echo FIXED > /var/backups/fix-sites-all-permsissions-2.3.6.txt
