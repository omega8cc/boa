...
update_solr () {
  # $1 is module
  # $2 is solr core path
  if [ ! -z $1 ] && [ ! -e "$2/conf/BOA-2.3.1.conf" ] && [ -e "/var/xdrago/conf/solr" ] && [ -e "$2/conf" ] ; then
...
setup_solr () {
  ...
  if [ -e "$_DIR_CTRL_FILE" ] ; then
    ...
    if [[ "$_SOLR_UPDATE_CONFIG_TEST" =~ "solr_update_config = YES" ]] ; then
      if [ "$_SOLR_CUSTOM_CONFIG_RESULT" = "YES" ] || [ -e "${_SOLR_DIR}/conf/BOA-2.3.1.conf" ] ; then
        _DO_NOTHING=YES
      else
...
add_note_platform_ini () {
  if [ -e "$_CTRL_FILE" ] ; then
    ...
    echo ";;  may not include all options available after upgrade to BOA-2.3.1"         >> $_CTRL_FILE
    echo ";;" >> $_CTRL_FILE
...
if [ -e "/var/run/boa_wait.pid" ] && [ ! -e "/var/run/boa_system_wait.pid" ] ; then
  ...
  if [ ! -e "/data/all/permissions-fix-post-up-BOA-2.3.1.info" ] ; then
    ...
    echo fixed > /data/all/permissions-fix-post-up-BOA-2.3.1.info
  fi
...
if [ ! -e "/var/backups/fix-sites-all-permsissions-2.3.1.txt" ] ; then
  ...
  echo FIXED > /var/backups/fix-sites-all-permsissions-2.3.1.txt
