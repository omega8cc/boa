...
add_note_platform_ini () {
  if [ -e "$_CTRL_FILE" ] ; then
    ...
    echo ";;  may not include all options available after upgrade to BOA-2.2.3"         >> $_CTRL_FILE
    echo ";;" >> $_CTRL_FILE
...
if [ ! -e "/var/backups/fix-sites-all-permsissions-2.2.3.txt" ] ; then
  ...
  echo FIXED > /var/backups/fix-sites-all-permsissions-2.2.3.txt
