...
disable_chattr () {
  if [ ! -z "$1" ] && [ -d "/home/$1" ] ; then
    ...
    if [ "$1" != "${_THIS_HM_USER}.ftp" ] ; then
      ...
      if [ ! -d "/data/disk/${_THIS_HM_USER}/.drush/usr/drupalgeddon" ] || [ ! -e "/data/disk/${_THIS_HM_USER}/static/control/.drupalgeddon.in.008.pid" ] ; then
        ...
        touch /data/disk/${_THIS_HM_USER}/static/control/.drupalgeddon.in.008.pid
      fi
      if [ ! -L "/home/$1/.drush/usr/drupalgeddon" ] && [ -d "/data/disk/${_THIS_HM_USER}/.drush/usr/drupalgeddon" ] ; then
