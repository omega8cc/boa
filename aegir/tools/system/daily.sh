...
add_note_platform_ini () {
  if [ -e "$_CTRL_FILE" ] ; then
    ...
    echo ";;  may not include all options available after upgrade to BOA-2.2.7"         >> $_CTRL_FILE
    echo ";;" >> $_CTRL_FILE
...
if [ -e "/var/run/boa_wait.pid" ] ; then
  ...
  if [ ! -e "/data/all/permissions-fix-post-up-BOA-2.2.7.info" ] ; then
    ...
    echo fixed > /data/all/permissions-fix-post-up-BOA-2.2.7.info
  fi
...
if [ ! -e "/var/backups/fix-sites-all-permsissions-2.2.7.txt" ] ; then
  ...
  echo FIXED > /var/backups/fix-sites-all-permsissions-2.2.7.txt
