...
d;};' /var/aegir/config/server_master/nginx/vhost.d/* &> /dev/null
  ...
  for _IP in `who --ips | cut -d: -f2 | cut -d' ' -f2 | sed 's/.*\/.*:S.*//g; s/:S.*//g; s/(//g' | tr -d "\s" | sort | uniq`;do _IP=${_IP//[^0-9.]/};echo "  allow                        $_IP;" >> /var/backups/.auth.IP.list;done
...
{
  for _IP in `who --ips | cut -d: -f2 | cut -d' ' -f2 | sed 's/.*\/.*:S.*//g; s/:S.*//g; s/(//g' | tr -d "\s" | sort | uniq`;do _IP=${_IP//[^0-9.]/};echo "  allow                        $_IP;" >> /var/backups/.auth.IP.list.tmp;done
