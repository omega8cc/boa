...
# =============================================================================
# xinetd.sh — xinetd service watchdog for BOA
# Split from: proc_num_ctrl.pl (legacy Perl service monitor)
#
# Starts xinetd when it is not running and the init script exists.
# Equivalent to the legacy line:
...
if [ -f "/etc/init.d/xinetd" ] && ! pgrep -x xinetd >/dev/null 2>&1; then
