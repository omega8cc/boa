...
# =============================================================================
# bind9.sh — Bind9 (named) service watchdog for BOA
# Split from: proc_num_ctrl.pl (legacy Perl service monitor)
#
# Restarts bind9 when the `named` daemon is not running, but only when the
# init script is present. Equivalent to the legacy line:
...
if [ -f "/etc/init.d/bind9" ] && ! pgrep -x named >/dev/null 2>&1; then
