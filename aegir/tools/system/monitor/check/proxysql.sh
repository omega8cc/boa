...
# =============================================================================
# proxysql.sh — ProxySQL service watchdog for BOA
# Split from: proc_num_ctrl.pl (legacy Perl service monitor)
#
# Restarts ProxySQL when the daemon is not running and the init script exists.
...
if [ -f "/etc/init.d/proxysql" ] && ! pgrep -x proxysql >/dev/null 2>&1; then
