...
# =============================================================================
# collectd.sh — Collectd service watchdog for BOA
# Split from: proc_num_ctrl.pl (legacy Perl service monitor)
#
# Starts collectd when it is not running and the init script exists.
...
if [ -f "/etc/init.d/collectd" ] && ! pgrep -x collectd >/dev/null 2>&1; then
