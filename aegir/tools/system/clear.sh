...
if_reinstall_curl() {
  ...
  if [[ ! "${isCurl}" =~ "OpenSSL" ]] || [ -z "${isCurl}" ]; then
    ...
    if [ -f "/usr/bin/curl" ]; then
      ...
      if [[ ! "${isCurl}" =~ "OpenSSL" ]] || [ -z "${isCurl}" ]; then
        echo "ERROR: /usr/bin/curl is still broken, uninstalling.."
        apt-get remove curl ${aptYesUnth}
      else
        echo "OK: /usr/bin/curl works"
      fi
    fi
    mkdir -p /var/opt
    rm -rf /var/opt/curl*
