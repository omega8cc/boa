...
Every caller passes `--rows=-1`, which turns mydumper's integer chunking off, so each table is dumped by one thread into one file (tables still dump in parallel across the threads). A fixed `--rows=N` puts mydumper into fixed-length chunking, where one step covers N key values of the first primary-key column rather than N rows, so it walks the whole MIN..MAX span of that key without re-anchoring on the next key that exists; a table keyed on a sparse bigint (timestamp-derived IDs) therefore produced one empty chunk file per step until the host ran out of inodes. Leaving the flag out is not the answer either: the adaptive chunker mydumper 0.21.3 then uses intermittently truncates a chunk's output file to zero bytes, losing every row it had written, while exiting clean with `metadata` written (a 60k-row INT-keyed table with one far-away id restored 1 row; 8 incomplete dumps in 100 on Percona 8.4, 8 in 30 on 5.7), which a nightly archive can never afford. `--rows=-1` is the disable form on the mydumper builds BOA pins (0.19.3 and 0.21.3). The same applies to the cluster variant, the `xoct`/`xcopy` exports and the per-site MyQuick dump provision takes where no backup mode is set (the hostmaster site's own backups, the safety copies ahead of Migrate and Delete, and a backend `provision-backup` run outside the task queue). A file-size bound for large single-table files, if ever wanted, is mydumper's `--chunk-filesize`, never `--rows=N`.
