...
| `nginx_fleet.sh` | every minute (`nice -n5 ionice -c2 -n7`) | Declare distributed crawler-fleet fingerprints from the access log and render the nginx `429` refusal maps (see [ABUSE-GUARD.md](ABUSE-GUARD.md)) |
...
| `updatesymlinks --auto-fix` | `:47` of hours 0-5, 22 and 23 (`nice -n5 ionice -c2 -n7`) | The nightly files-symlink auto-fix; retried hourly through the night, self-skips a busy hour and stamps once the two-step has run |
| `migration_proxy_certs.sh` | 06:45 daily (`nice -n5 ionice -c2 -n7`) | A no-op unless this box proxies a migrated account: pulls the target's renewed certificates so a long-lived proxy never presents an expired one |
| `graceful.sh` | 03:01 daily | Graceful service cycle |
| `backboa backup` / `duobackboa backup` | 03:15 / 05:15 daily | Off-site backups |
| `killer` (a line in `/etc/crontab`, not the root crontab; re-written on every root `barracuda` run) | every minute | `kill -9`s any `apt update` / `apt-get update` older than 99 s, logging to `/var/log/boa/.proc.forced.kill.exceptions.log`; stands down while `/etc/boa/.pause_tasks_maint.cnf` exists |
...
|---|---|
| `aegir/tools/system/minute.sh` → `/var/xdrago/minute.sh` | Per-pass fan-out of the per-service watchdogs (`monitor/check/{system,unbound,valkey/redis,mysql,php,fpm_tune,sqlprobe,xdr9000,batch_guard,task_guard,autosync,nginx,nginx_guard,java}.sh`) |
...
This is **by design**: a dead `php-fpm` or a load spike is caught within ~5 seconds, not at the next whole-minute tick. But it is also the **dominant idle-load source on small boxes**.

Roughly twenty short-lived processes are spawned every ~5 seconds, each re-sourcing `/root/.barracuda.cnf` and running `pgrep` scans. Before the box-class throttle existed, that churn alone held an idle 2 CPU / 4 GB test VM, with the Ægir queue and site cron switched off, at a load average of about 3–4 (observed in June 2026). The cost is fork/exec and config re-parsing, not real work — which is why only *disabling cron* (not tuning any single watchdog) dropped the load to ~0. That is what the box-class throttle below tames.
...
This is why a slow tick never stacks into a fork storm — a new cron tick that finds the previous one still working simply exits. The auth scanners use their own PID-checked `noclobber` lock instead, to the same end.
...
`minute.sh` runs `_launch_auto_healing` once per pass, which `nohup`-spawns each per-service watchdog concurrently. Each checks one service and, if it finds it down, stuck, or misconfigured, repairs it in place. The Valkey/Redis choice is made by an init-script probe: `valkey.sh` runs if `/etc/init.d/valkey-server` exists, else `redis.sh` if `/etc/init.d/redis-server` exists. `fpm_tune.sh`, `sqlprobe.sh`, `xdr9000.sh`, `batch_guard.sh` and `task_guard.sh` run only if their files are present; `autosync.sh` runs only while its file AND the operator-armed `/root/.xmass.autosync.cnf` marker are both present, so the rest of the fleet pays one file test per pass.

| Watchdog | Guards | Action |
|---|---|---|
| `system.sh` | OS-level health: SSHD, postfix, rsyslog (the restart verified: a rsyslogd that cannot start, a missing `/etc/rsyslog.conf` say, is logged and alerted as `restart FAILED` with its own reason, never as restarted), cron duplicates (the daemon counted by its own anchored command line, `/usr/sbin/cron` with or without arguments, rechecked after 3 s, killed by the same pattern, the processes counted logged before the restart — a command line that merely names the path is neither a second cron nor collateral), LFD (alive means the pid in `/run/lfd.pid` is running and titled `lfd`, so a stray process merely named after it, a `tail -f lfd.log`, never masks a dead daemon; and a firewall left disabled with `csf -x`: re-enabled once the disable marker is ten minutes old and no BOA pass is in flight, the lfd start verified before it is logged), FTPS (pure-ftpd), vnstat, gpg-agent / dirmngr pile-ups, ClamAV, DHCP lease, and system memory pressure. Every daemon here is judged alive the same way, by the pid in its pidfile carrying the daemon's process name, and the pile-up and duplicate counters count exact process names, so a command line that merely mentions a daemon neither masks a dead one nor inflates a count | Restarts/starts the down service; on sustained memory pressure kills the single largest process that is safe to kill (below); culls runaway `wkhtmltopdf` between 5–10% free; forces logrotate; rebuilds DHCP allow rules in `csf.allow`; cooldown-gated per service |
| `unbound.sh` | Local DNS resolver: liveness by exact process name under the `unbound` user plus the PID file, a live `host files.boa.io` lookup against `127.0.0.1`, duplicate masters (counted the same way, rechecked after 3 s, the masters counted logged before a restart — `unbound-control`, `unbound-anchor` or a tool naming `/usr/sbin/unbound` never count), `/etc/resolv.conf` sanity; and, only on a box whose `/etc/default/unbound` carries `always_nxdomain` (a test/CI outbound-mail blackhole, never for production), the `ci-nomail.conf` drop-in | Restarts unbound (cooldown-gated) and checks that the restart left a daemon behind: when it did not, the incident log gets `ALRT: Unbound did not start after the restart:` with the first error `unbound-checkconf` reports, and says so when `/usr/sbin/unbound` is a distro build rather than BOA's (a distro package upgrade replaced it; that build reads `/etc/unbound/unbound.conf`, cannot start on BOA's config, and the next barracuda pass reinstalls BOA's); rewrites a BOA-tagged `/etc/resolv.conf`; on an armed box writes and re-asserts `unbound.conf.d/ci-nomail.conf` (NXDOMAIN for sendgrid.com, sendgrid.net, mailgun.net, sparkpost.com, postmarkapp.com, brevo.com and the WHOLE of amazonaws.com), adds the `include-toplevel` line and reloads unbound whenever the zones fall out of `unbound-control list_local_zones` |
...
| `redis.sh` | **Legacy, kept for boxes still on Redis**: it ships with the release tree, so every install and every pass carries it (a pass copies the whole tools tree into `/var/xdrago`), but it is not fetch-refreshed between passes; the init-script dispatch launches it only where `/etc/init.d/redis-server` exists and no `valkey-server` does — legacy pre-Valkey logic (PING restart, `/var/lib/redis/*` wipe, FPM reload, plan-gated `run-redis-restart.pid`) |
...
| `php.sh` | PHP-FPM (all installed versions, `56`–`85`): master + `wwwNN.fpm.socket` + PID liveness, duplicate masters, `already listen on` / `Address already in use` conflicts, per-pool `max_children` saturation (new error-log hits), giant logs, oversized `fastcgi_temp`, a missing `/var/log/php/phpNN-fpm-error.log` for an installed version (graceful reload of THAT version only, which clears its APCu; own stamp `/run/phpNN-fpm.logfix.cooldown`) | Per-version restart on a sustained, double-checked failure (cooldown-gated); cleans `fastcgi_temp`; honors the plan-gated (or box-wide `_ALLOW_PHP_FPM_RELOAD=YES`) `run-php-fpm-reload.pid` APCu-clear sentinel; spawns `segfault_alert.pl` (unless `/root/.high_traffic.cnf` or `/root/.giant_traffic.cnf` is set) |
...
| `autosync.sh` | Standing-mirror file cadence driver — the file half of an HA mirror whose database half replicates live. Runs only while an operator has armed `xmass autosync <target-ip>` on this ACTIVE box (marker `/root/.xmass.autosync.cnf`; one-way, active → mirror, per the de-sync rule that nothing moves a mirror except the active). This file owns only the cadence: the marker-gated launch, the early self-throttle to the EFFECTIVE window (the LONGER of the armed `_XMASS_AUTOSYNC_EVERY`, floor 5 min, and the duty-cycle floor: a pass may occupy at most one part in `_XMASS_AUTOSYNC_DUTY + 1` of wall clock — box-wide knob read from `/root/.barracuda.cnf`, default 3, so the box rests at least three times the last measured pass before the next one starts — and the guard only ever slows a pair down; ~every non-due tick ends at that check), the `ionice`/`nice` wrapping, the hard per-pass `timeout -k` (`_XMASS_AUTOSYNC_TIMEOUT`, default 21600 s — killing a hung pass is safe: the legs are idempotent delta rsync and the verb lock's signal trap exits), the size-bounded pass log `/var/log/boa/xmass.autosync.log` (never trimmed under a live pass), the incident mail, and the staleness alarm (no pass COMPLETED for six times that effective window, floor 180 min → one daily latched warning + mail, whatever the silent-defer reason). Every safety decision lives in `xmass autosync --run` itself (phase, roles, the single-flight owner-PID lock in defer mode, live barracuda/octopus evidence, the target's standby marker verified at pass start and re-proved at every leg boundary, fail-closed on transport) — deliberately no second copy of any gate here to drift. A box that is itself a standby or a finalized proxy stands down with a daily latched warning plus one daily mail | Invokes `bash /opt/local/bin/xmass autosync --run` at the effective cadence and mails a throttled incident report when a pass fails (a deferred pass — lock held, upgrade in flight, cutover window — is silent rc 0 by design: the next tick retries, and the staleness alarm backstops every silent class). `_USE_XMASS_AUTOSYNC=NO` parks the driver without touching the marker; `--force --stdout` is the operator debug form |
| `xdr9000.sh` | *Read-only* permanent archive harvester (XDR9000) — no restart, no ban, no config change; `_XDR9000=NO` stops recording and keeps the archive | Copies the attack-ban ledgers (web/SSH/FTP), thirteen incident logs (the twelve auto-heal subsystems and the nightly backup), backup outcome archives and the fpm-tune/sqlprobe/load-profile corpora into the append-only JSONL archive under `/var/log/boa/xdr9000/` before their routine reapers run; adds a metric sample every ~5 min and a box-facts record hourly; rolls up daily totals and gzips past months once per date change. The archive survives upgrades and log rotation by construction and is never pruned; the root-only `xdr9000` CLI reads it (see [XDR9000.md](XDR9000.md)) |
| `nginx.sh` | Nginx: master + `/run/nginx.pid` liveness, multiple masters, zombie/stopped master/worker states, `Cannot allocate memory` (OOM), `Address already in use`. A missing pidfile alone, with a live master still serving, is an artefact and stands down; a missing master always heals (workers inherit the listen sockets and serve headless) | Graceful escalation on every restart path, all cooldown-gated: QUIT to the recorded master (verified to still be nginx), a bounded wait, `-9` only for survivors, then `service nginx restart` (rotating `error.log`). Honors the plan-gated (or box-wide `_ALLOW_NGINX_RESTART=YES`) `run-nginx-restart.pid`; a request landing inside a cooldown is kept for the next pass. After any of its restarts leaves no master while `/etc/nginx/main.d/quic_bpf.conf` is in place, it sheds that fragment (on a BPF failure in the fresh error log, on a binary that no longer knows the directive, or, at most once in ten minutes, by trial: start without it and keep it shed only if that made the difference), leaves the latch `/etc/nginx/main.d/quic_bpf.failed`, starts Nginx again and records the incident; see [NGINX-QUIC-BPF.md](NGINX-QUIC-BPF.md). The throwaway master a barracuda pass runs for a second or two to probe `quic_bpf` is never counted as a master. |
| `nginx.sh` / `mysql.sh` / `system.sh` (standby duty) | On a box carrying `/root/.standby.cnf` (a passive replication mirror) the same watchdogs ENFORCE the holds instead of healing: `nginx.sh` keeps the web tier DOWN (stops a running nginx, kills pure-ftpd on sight, asserts the `BOA_STANDBY_WEB` firewall chain, and removes the chain again the moment the marker goes), `mysql.sh` keeps the replica's `super_read_only` locked (persisting it in `xmass_gtid.cnf`, retrofitting mirrors built by older bytes, and releasing it — runtime verified before the persisted block is stripped — once the marker goes), and `system.sh`'s FTPS healer stands down on the marker alone, as the pure-ftpd kill does (both hold right through an xmass window, and `/root/.standby.serve.cnf` does not exempt them either: no cutover step needs FTPS, and removing the standby marker releases it within one monitor pass). The web hold follows the DATABASE, not the xmass window: `nginx.sh` releases it only on a box that reads as promoted (a clean role probe, no replica config, `super_read_only` 0), latched in `/var/log/boa/.standby_promoted.pid` so an ambiguous read changes nothing and absent means held — an init or a re-init never opens a mirror's web tier. `mysql.sh` alone still stands down while an xmass window is in flight (`/run/boa_xmass_init.pid` or its reboot-proof twin `/root/.standby.init.pid` fresh), and it converts a mirror built by older bytes wherever the hold block is missing under the marker, except on a box the cutover's step 11.5 unlocked (it leaves a line saying so in `xmass_gtid.cnf`); it honours the window signal ten minutes longer than `second.sh`, whose removal of a promoted box's leftover marker at the 48-hour ceiling must land first. `/root/.standby.serve.cnf` opens the web tier only (read-only preview; one log line an hour while it holds) | The holds, not heals: transition-logged to the same `.incident.log` files |
...
| `java.sh` | JVM services: Jenkins, Solr 9 / Solr 7 / Solr 4 (Jetty 9), and Jetty `Address already in use` | Restarts the down/stale-PID service (cleaning its `/tmp` scratch and rotating logs). Only that restart work is skipped while a BOA/Octopus run is in progress (or under `/run/max_load.pid`, `/run/critical_load.pid`, `/run/boa_java_auto_healing.pid`). ABOVE that gate, on every pass whatever is running, it enforces the xmass Solr hold: while `/var/log/boa/.xmass_solr_hold.pid` exists it clears the exec bit and boot links of solr9/solr7/jetty9/solr4, reaps the solr9/solr7/jetty9 JVMs, records what it disarmed in `/var/log/boa/.xmass_solr_disarmed.list` and exits (a barracuda pass re-arms the init scripts and starts Solr, which is why the hold is enforced here rather than skipped); on a `/root/.standby.cnf` box without the hold it exits without starting or re-arming anything; once the marker is gone it re-arms exactly the recorded set |

A few load-bearing details worth calling out:

- **`system.sh` answers a memory emergency by freeing memory, not by taking the site down.** Pressure is read from `MemAvailable` and the ≤5% condition must hold across three samples five seconds apart, standing down the moment one recovers.

  The response is a single `kill -9` of the largest resident process that is safe to kill; `mysqld`, `sshd` and the provision/backup chain (`provision`, `drush`, `mydumper`, `duplicity`, `aegir.sh`, `backboa`, `multiback`) are never chosen, because killing one of those means a broken task or a corrupt dump rather than freed memory. If everything large is exempt it logs, pages, and leaves the rest to the kernel OOM killer. Between 5% and 10% free it only culls runaway `wkhtmltopdf` (more than 2 instances).

  Repeats are paced by `_OOM_COOLDOWN_SECS` (60 s) and capped by a breaker that latches `/run/boa_oom_latched.pid` after `_OOM_FLAP_MAX` (3) kills inside `_OOM_FLAP_WINDOW_SECS` (3600 s), pages once, and waits for an operator or `_OOM_FLAP_LATCH_MINS` (60). This replaced a cascade that killed seven services within three seconds of a single `free -mt` sample, wiped the cache server's data, restarted Percona through the whole-stack path and never restarted the web tier it had killed.
- **`mysql.sh` only restarts a database that will not answer.** A needless MySQL restart is itself an outage, so the test is functional: a `mysqladmin ping` reply of any kind proves the server is alive, an authentication error or `Too many connections` included, and a first failed probe is re-tried across a short grace before the server is called down. That grace matters where `mysqld_safe` supervises Percona, since it respawns a dead `mysqld` within a second or two and the socket and PID file are legitimately absent meanwhile.

  A busy-but-responsive server is therefore never restarted: high load with a high thread count (`_LOAD_THRESHOLD` 33.0 *and* `_THREAD_THRESHOLD` 99) and a `Too many connections` flood both fall through to the real remedies, `flush-hosts` (issued as `FLUSH NO_WRITE_TO_BINLOG HOSTS`, so it never replicates: the bare form minted one errant own-UUID transaction per watchdog pass on a 5.7 replica; on 8.4, where `FLUSH HOSTS` is gone, it fails silently) and the long-query killer.
...
- **`sqlprobe.sh` never tunes anything live either.** It is the SQL-side sibling: the data collector behind the `memorytuner` advisory, which in turn only *names* `_SQL_*_FORCE` pin lines — applying them stays a human decision.

  Its three side stores (the daily datadir measurement, the Valkey used-memory peak taken only after an hour of cache uptime, and the Valkey demand window — hits, misses and evictions accumulated from deltas between two same-run samples an hour or more into that run, plus the high-water live occupancy seen within the window; the 7-day rotation decays the counters by halving rather than zeroing them and re-anchors the peak to the current sample, while the control loop clears the whole window whenever it acts, so each step is judged on post-action evidence only) keep the automatic tuning's own stores fresh between weekly upgrades; a box that opts out with `_USE_SQLPROBE=NO` simply falls back to measuring at tune time.

  The demand window is the primary input to the cache-ceiling control loop in the tune pass, which steps the ceiling from hit rate and eviction behaviour rather than from occupancy; the older peak-times-four rung survives only as its fallback, so an opted-out box reverts to it once the window goes stale.

  `memorytuner` also renders a per-box needs ledger from the records carrying the ledger keys — measured per-consumer demand with stated margins, an OS reserve derived from residual RSS, and file cache treated as the budget remainder — and that ledger is observation only: it names no pin and applies nothing. While a demand window exists the advisory reports the self-managed ceiling and recommends against `_VALKEY_MAXMEM_FORCE`, since pinning it disables the loop.
- **`batch_guard.sh` can only kill what is provably dead — and load level is never consulted, in either direction.**

  It arms on the storm's own two-floor signal (`/bgp-start/`-specific own-IP 499 volume + many distinct looping bids — the knobs above), never on load: a pool-sized storm pins one FPM pool and degrades that site while a many-core box's average stays under any workable per-core floor (observed live at 0.69/core against the old 1.0/core gate, for 40+ minutes, blind), and in the other direction it never waits for calm either — its heal is cheap SQL, not a restart.

  Age is never a criterion (D7 writes `{batch}.timestamp` once, at creation, so a long-running live batch and an abandoned one look identical by age); a bid heals only if it is looping through `/bgp-start/` right now, is driven by a live `background_process` row (which is what the module's cron re-launches — browser and update.php batches never have one, so they are structurally unreachable, and forged request walls cannot make a victim's batches eligible), and its blob fingerprint plus queue count survived at least eight minutes unchanged — with the final DELETE conditional on all three of those same values.

  It also never gates on the `/run/max_load.pid` tier markers: second.sh sets those even on passes where it declines to pause, so a marker gate would disarm the guard for whole backup nights.

  What it requires instead is **service continuity**: no candidate confirms unless nginx, every FPM master and mysqld have been up, uninterrupted, since before its first sighting — pidfile birth times and the server's own Uptime are the witnesses — because a teardown (the load auto-pause, a watchdog heal, a DB restart) freezes every blob and would make a live batch read as dead; a teardown that came and went entirely between two passes still invalidates the interval.

  Knobs: `_BATCH_GUARD_499_MIN` (200) + `_BATCH_GUARD_STORM_BIDS_MIN` (12) — the two arm floors — `_BATCH_GUARD_BID_MIN` (20), `_BATCH_GUARD_COOLDOWN_SECS` (600), breaker `_BATCH_GUARD_FLAP_MAX`/`_BATCH_GUARD_FLAP_WINDOW_SECS`/`_BATCH_GUARD_FLAP_LATCH_MINS` (3/7200/60, latch `/run/boa_batch_guard_latched.pid`); `--detect-only` is the operator dry run and beats any cnf line.
- **`task_guard.sh` can only fail what is provably dead — and it never re-runs anything.** The judgement is the process table: an orphaned PROCESSING row is touched only when it is past the grace, is the task's current revision, and its instance user has no live task runner and no live provision backend — one live process holds the whole instance for that pass, because with per-user evidence the guard never guesses which row a process belongs to (the failure direction is a delayed heal, never a killed live task).

  The reset is per-vid and conditional on the row still being PROCESSING, so the frontend's own dispatcher reaper (on current hosting releases) and the manual button always win a race. It marks rows failed with a truthful log entry; re-running a crashed task is the operator's call — an auto-`--force` of a crashed migrate or clone is exactly the class of blind re-fire BOA never does.
...
|---|---|---|
| `_second_flood_guard` | more than 4 live `second.sh` processes | log to `/var/log/boa/sec-count.kill.log`, kill them |
| `_csf_flood_guard` (csf) | more than 4 live `/usr/sbin/csf` processes | log to `csf-count.kill.log`, kill them, then `csf -tf` + `csf -df` to flush temp/deny bans |
| `_csf_flood_guard` (fire) | more than 7 `guest-fire.sh` processes | log to `fire-count.kill.log`, flush csf temp bans, kill them; at more than 9, log to `fire-purge.kill.log` and purge the deny rules too |

Each guard counts and kills on the target's anchored EXECUTION FORM only, the kill using the very pattern the count used: `^(/bin/)?bash (-c bash )?/var/xdrago/second\.sh( |$)`, `^(/usr/bin/perl )?/usr/sbin/csf( |$)`, `^(/bin/)?bash (-c bash )?/var/xdrago/guest-fire\.sh( |$)`. A command line that merely names the script (an editor, a grep, csf's hourly integrity sweep, an operator's own shell) is neither counted nor killed.
...
5. **Keeping table repair out of the way.** `sqlcheck.sh` and `checksql.sh` run a full-server `mysqlcheck` that takes locks across every database, so both decline while a backup, another maintenance operation, an auto-heal or a restart is in flight. `checksql.sh` always executes the repair script it generates: `_INCIDENT_REPORT=OFF` (or no `_MY_EMAIL`) silences the two mails about it, never the repair. The script that ran is left at `/var/xdrago/acrashsql.sh`. `mysql_repair.sh` holds `/run/boa_sql_maintenance.pid` for its own run so the watchdog cannot read that work as a fault.
...
When a watchdog takes a corrective action it appends to `/var/log/boa/<service>.incident.log` and may e-mail `_MY_EMAIL`, gated by `_INCIDENT_REPORT` (from `/root/.barracuda.cnf`). The **send threshold differs by watchdog**: in `php.sh`, `nginx.sh` and `java.sh` the e-mail is sent **only when `_INCIDENT_REPORT` is `ALL`** — their lower settings still log but mail nothing.

`unbound.sh` keeps its routine restart notice `ALL`-only, but a resolver that does not come back from a restart is an `ALERT` that `MINI` and `CRIT` both hear, once per six hours (class `unbound-dead`): auto-healing cannot fix that state, and while it was silent a box could resolve through the fallback servers in `/etc/resolv.conf` for days with nobody told. `mysql.sh` sends on anything other than `OFF`. `system.sh` implements the `OFF` / `CRIT` (= ALERT-only) / `ALL` ladder.

`valkey.sh` implements the full four-step ladder: a genuine cache restart mails at the default `MINI` and up — a cache restart empties the cache, and while that mail was ALL-only an operator learnt about a flapping cache server from the load graphs — the circuit-breaker page is an `ALERT` that `CRIT` still hears, and informational notices stay `ALL`-only. (`redis.sh`, retired from delivery, keeps its old ALL-only gate on boxes that still run it.)

In every case the corrective action and the `.incident.log` entry happen regardless of the e-mail setting. A watchdog also sends **one alert per class of incident per `_INCIDENT_EMAIL_COOLDOWN_SECS`** (30 min) rather than one per pass, and says in the subject how many it stood in for; nothing is lost, because every incident is still logged and the mail body is the tail of that log.

Classes that are not the same incident keep separate cooldowns — the database circuit breaker, backup contention, anything `system.sh` raises at `ALERT`, the valkey restart (`valkey`) / alert (`valkey-alert`) / circuit (`valkey-circuit`) classes, and `second.sh`'s pause/terminate pages (`second`; a terminate-and-pause pair is one incident, so both share the key and the second mail is held with the first carrying it) — so a cheap alert cannot spend the budget an expensive one needs.

Reporting is additionally suppressed during the post-boot uptime grace period (see Load control), so a reboot does not spray "service was down, restarted" mail while the stack is still coming up.
...
- **CRIT — terminate runaways, then pause.** Above 810%, `_terminate_processes` runs **first** — a stuck PHP request, a Drush job in a loop, a wget/curl pulling something huge — *before* `_hold_services` pauses the web tier. Killing the runaway first is what lets the box recover instead of immediately re-spiking after the pause.
  ...
  The kill is TERM-first with a two-second grace before `-9` reaches only the survivors: a Drush job SIGKILLed mid-write leaves half-applied state on disk that no rollback repairs. The targets are matched by process name (`php`, `wget`, `curl`; a Drush job is a `php` process). The terminate step stands down entirely during a BOA pass: while `/run/boa_run.pid`, `/run/boa_wait.pid` or `/run/octopus_install_run.pid` exists it returns before any kill, because a pass's own PHP builders and mirror fetches are the busiest processes on the box. The pause that follows is not covered by that hold.
...
- **A backup is in progress** — `_backup_in_progress` matches the BOA backup orchestrators by path (`backboa` / `duobackboa` / `multiback` / `mysql_backup.sh` / `mysql_cluster_backup.sh`) or the dump and restore engines by exact name (`mydumper`, `xtrabackup`, `xbstream`, `myloader`, `myloader.bin`), or `duplicity` by an anchored command-line pattern (it runs as its pipx Python, never as a process named `duplicity`), or finds `/run/boa_sql_cluster_backup.pid`.
...
The MAX/CRIT markers are **latched**: once `/run/max_load.pid` or `/run/critical_load.pid` is set, `_clear_pause_latch` removes them only after **both** the 1-minute and 5-minute per-CPU loads fall below `_CPU_RESUME_THRESHOLD` (= `_CPU_MAX_THRESHOLD` × `_RESUME_FRACTION`, default fraction **0.8** → 80% of the MAX threshold). The service watchdogs restart web only when both markers are absent, so this holds the web tier paused through the dip band just under MAX instead of un-pausing the instant load ticks below the threshold — without it a box hovering at the line flapped pause/resume every pass.

`_RESUME_FRACTION` is overridable in `/root/.barracuda.cnf` and is clamped to the open interval (0,1): a value of 0 (or a typo sanitising to a bare `.`) would leave the latch never clearing, and a value ≥ 1 would disable the hysteresis, so anything outside (0,1) resets to the default.

The threshold alone cannot see one kind of flap: a pause that stopped the web tier collapses load toward zero, and a single below-resume reading is exactly what that collapse produces — it proves the web is off, not that the pressure is gone. So the clear takes **`_RESUME_HOLD_PASSES` (default 3, clamped 1–60) consecutive below-resume evaluations**; the streak is persisted in `/run/second_resume_streak` so it spans the script's one-minute lifetimes, goes stale after a quiet gap, and resets whenever the calm is interrupted.

The clear also **stands down while a database restart is in flight**, on the same two markers and `_SQL_MUTATION_MAX_MINS` recency bound the web watchdogs use — a latch cleared mid-restart would only queue the herd against a database that is not back yet. Resume is evaluated at the end of **every** pass, whatever load tier ran: a MAX/CRIT pass whose post-sleep re-check just-missed its tier used to run nothing at all, stranding a latch set a minute earlier.

Both the held and the cleared latch log one line to `high.load.incident.log`. The latch still **fails toward resume** — a missing or unusable threshold clears the markers after the minimum hold rather than leaving web stuck paused.
...
- **Hard stops first.** The replication-standby marker `/root/.standby.cnf` is checked first, above the proxy and pause gates (an ha-switch failback target carries both markers): the runner exits and parks the Ægir master crontab, and the first pass without the marker un-parks it. The queue-stop file `/run/boa_queue_stop.pid` exits the whole script and is honoured again per child dispatch. Last comes the disk gate: `boa archived heal all` runs whenever `/` or any instance's `static/files/.archived` store is above the heal threshold (85% by default), and nothing is dispatched while the root filesystem is above 90%.

  Besides those, it exits immediately if `/root/.proxy.cnf` (rightful: a proxy has no local sites for tasks), `/etc/boa/.pause_tasks_maint.cnf`, the PHP-idle quiesce marker `/run/boa_php_idle_quiesce.pid` (owner-PID keyed, dead-owner self-cleaning), or a `max_load`/`critical_load` pid is present; and again if too many `runner.sh` instances are already running, or a SQL backup, `owl.sh`, a MySQL restart/cluster-backup, or `boa_cron_wait.pid` is in flight.
...
The order matters: a CI box is CI even if it is also small; otherwise the ≤ 4 GB RAM check classifies the box SLOW even if `runner.sh` has not yet written `.slow.cron.cnf`.
...
- [NGINX-QUIC-BPF.md](NGINX-QUIC-BPF.md) — HTTP/3 across Nginx reloads: the probed `quic_bpf` fragment, its latch, and what `nginx.sh` does when a start fails with it in place.
