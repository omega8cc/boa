...
| `nginx.sh` / `mysql.sh` / `system.sh` (standby duty) | On a box carrying `/root/.standby.cnf` (a passive replication mirror) the same watchdogs ENFORCE the holds instead of healing: `nginx.sh` keeps the web tier DOWN (stops a running nginx, kills pure-ftpd on sight, asserts the `BOA_STANDBY_WEB` firewall chain, and removes the chain again the moment the marker goes), `mysql.sh` keeps the replica's `super_read_only` locked (persisting it in `xmass_gtid.cnf`, retrofitting mirrors built by older bytes, and releasing it — runtime verified before the persisted block is stripped — once the marker goes), and `system.sh`'s FTPS healer stands down. The web-tier holds stand down while an xmass window is in flight (`/run/boa_xmass_init.pid` or its reboot-proof twin `/root/.standby.init.pid` fresh); the pure-ftpd kill does NOT (it holds on `/root/.standby.cnf` alone right through that window, and `/root/.standby.serve.cnf` does not exempt it either: no cutover step needs FTPS, and removing the standby marker releases it within one monitor pass), and `/root/.standby.serve.cnf` opens the web tier only (read-only preview; one log line an hour while it holds) | The holds, not heals: transition-logged to the same `.incident.log` files |
