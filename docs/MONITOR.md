...
5. **Keeping table repair out of the way.** `sqlcheck.sh` and `checksql.sh` run a full-server `mysqlcheck` that takes locks across every database, so both decline while a backup, another maintenance operation, an auto-heal or a restart is in flight. `checksql.sh` executes the repair script it generates only when `_MY_EMAIL` is set and `_INCIDENT_REPORT` is not `OFF`, because that run sits inside the mail block; otherwise it still writes `/var/xdrago/acrashsql.sh` and touches `/var/log/boa/last-run-acrashsql`, and the repair is the operator's to run. `mysql_repair.sh` holds `/run/boa_sql_maintenance.pid` for its own run so the watchdog cannot read that work as a fault.
