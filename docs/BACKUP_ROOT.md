...
The global backup system is designed to securely back up system-wide data and ensure data integrity and recoverability. It uses **Duplicity** to create encrypted, incremental, and versioned backups stored in remote cloud services. Coverage is split across two path-sets generated by the system: the `global` set (`/etc`, `/opt/solr4`, `/var/aegir`, `/var/solr7`, `/var/solr9`, `/var/www`, `/var/xdrago`, plus `/data/disk/arch` and selected `/var/backups/*` added via include globs) and the separate `data` set (the per-platform `/data/disk/*` directories, `/data/all`, `/data/conf`, and `/home`).

### **Features**
- **Global Scope**: Backs up critical system directories across two path-sets — the `global` set covers `/etc`, `/opt/solr4`, `/var/aegir`, `/var/solr7`, `/var/solr9`, `/var/www`, `/var/xdrago` (plus `/data/disk/arch` and selected `/var/backups/*` via include globs), and the `data` set covers `/data/disk/*`, `/data/all`, `/data/conf` and `/home`.
...
1. **Global Backup Scope**:
   - Covers directories crucial for system operations, split across two generated path-sets:
   - `global` path-set (`_SOURCE`):
     - `/etc`
     - `/opt/solr4`
     - `/var/aegir`
     - `/var/solr7`
     ...
     - `/var/www`
     - `/var/xdrago`
     - plus `/data/disk/arch` and selected `/var/backups/*` added via include globs (not `_SOURCE`)
   - `data` path-set (generated include list; `_SOURCE` is empty):
     - `/data/disk/*` (per-platform directories)
     - `/data/all`
     - `/data/conf`
     - `/home`
