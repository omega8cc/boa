...
;;  hide it for a few seconds, and a newly created file can take up to three
;;  minutes to be noticed. A .dev. alias of a site cuts the front cache
;;  to one second.
;;
```
...
;;
;;  Drupal 6/7 and Backdrop only: Drupal 8+ re-applies its own session
;;  lifetimes (core defaults or the site's services.yml) when a session starts.
;;
...
;;
;;  BOA default defined in the system level global.inc file is 86400 == 24h.
;;  Drupal 6/7 and Backdrop only, as above.
```
...
;;
;;  Example (Drupal 6/7): redis_exclude_bins = "cache_form,cache_foo,cache_bar"
;;  Example (Drupal 8+):  redis_exclude_bins = "foo,bar"
;;  Drupal 8+ names its bins without the cache_ prefix (foo for cache_foo).
```
...
### INI (platform level) for Redis / Valkey Cache Reliability and Observability
