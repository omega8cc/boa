...
account created after this ships is born converted. Each conversion writes
`/data/disk/oN/log/instance-group.txt`.
...
account rather than half-convert it, and refuses while any fetched root-run
writer on the box still carries the box-wide form (`instgrp check`): an old
`fix-drupal-*` script or nightly worker would write `users` back on its
next pass, and an old `websh` would lock the tenant out of its shell.
...
instgrp convert oN | all [--from-octopus]
instgrp revert  oN
instgrp check
```
...
under the account's roots, and one verdict line — `CONVERTED`,
`UNCONVERTED`, `DRIFT` (converted, but paths are back in group `users`: an
import from an unconverted box, a hand `chown`, a tool predating the form;
re-run `convert`, it only touches what drifted) or `INCONSISTENT`. Exit
status 0 / 0 / 2 / 3 in that order. Every action logs one line to
...
line is born the old way. It does not undo a conversion already made — run
`instgrp revert oN` for that. An explicit `instgrp convert` ignores the
switch; it is the operator's order. The key is persisted per instance
(written at install, appended with its default on upgrade when absent).

`boa cleanup purge oN` removes the account's group after its identities.
Removing a single sub-account leaves the group in place: it belongs to the
account.
...
- Never write the literal `:users` onto an account's tree by hand
  ...
  to every other tenant until the next nightly pass, and `instgrp status`
  reports it as `DRIFT`. Use `chown -R oN:$(id -gn oN) …`, which is right
  on both models. See `FIXREPO.md`.
- Backups, restores, clones and migrations need no conversion step: the
  extracting process is the account's backend user, so the restored files
  take the account's primary group.
- GIDs are allocated per box from the system range, so the same account
  ...
  `xoct transfer` therefore runs a group pass on the destination after
  every transfer (paths in no group or in `users` take the destination
  account's group, `users` while unconverted) and drops the conversion
  marker that travelled with the tree -- it recorded the source box's
  conversion. `instgrp` reads a marker whose gid is not the account group's
  gid on this box as STALE (ignored), and `convert` claims any path of the
  account's roots that is in no group.
