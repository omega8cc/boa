...
5. Install Barracuda and Octopus.
   ...
   **NOTE:** BOA installs **Percona 5.7 by default**, and **Drupal 11 needs MySQL 8** -- on BOA that means **Percona 8.4**. To host Drupal 11, or any distribution built on it (Drupal CMS, Commerce Kickstart, Thunder, LocalGov, farmOS, OpenCulturas, Varbase), pick one of the two ways:
   ...
   - Upgrade a Percona 5.7 server later, in two steps: `barracuda up-lts system percona-8.0`, then `barracuda up-lts system percona-8.4` (`up-pro` on a PRO server). The upgrade runs in place on the same server, with no migration. There is no direct jump, it works on Devuan Daedalus, and it wants a whole-server snapshot and a `codebasecheck --box --deep` run first -- see [docs/MIGRATE-PERCONA8.md](https://github.com/omega8cc/boa/tree/5.x-dev/docs/MIGRATE-PERCONA8.md).
