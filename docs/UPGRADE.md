...
An older launcher used to stamp the new tree with its own release, and the Octopus half of the upgrade then refused to run. Every such run must be able to read that launcher: if it cannot be fetched complete or its release cannot be read, the run stops with a message before it fetches the tree, even when no refresh is due; a due refresh that cannot be installed, or a launcher still older after refreshing itself once, stops it the same way.

Only a release difference triggers a refresh, only launchers that carry the check do it (a box restored from an image made before it still needs `BOA.sh.txt` first), other verbs (`php-idle`, `up-distro`) skip it, and a launcher newer than the tree you asked for is left as it is.
