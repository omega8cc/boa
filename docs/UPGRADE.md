...
All standard non-major system upgrades can be run with **BARRACUDA** and all Ægir instances can be upgraded with **OCTOPUS**, but we highly recommend to use the fully automated procedure explained in Self-Upgrade How To: [docs/SELFUPGRADE.md](https://github.com/omega8cc/boa/tree/5.x-pro/docs/SELFUPGRADE.md)
...
The `BOA.sh.txt` step stays the documented first step: it refreshes every tool, including launcher fixes shipped under the same release. On top of it, on an `up-lts`, `up-pro` or `up-dev` run, `barracuda` checks its own release before it fetches the tree: it reads the release of the launcher the tree you asked for publishes, and when its own release is older (a box upgraded before its tools refreshed) it replaces itself with that launcher and runs again with the same arguments, printing two `NOTE:` lines that say so. An older launcher used to stamp the new tree with its own release, and the Octopus half of the upgrade then refused to run. Every such run must be able to read that launcher: if it cannot be fetched complete or its release cannot be read, the run stops with a message before it fetches the tree, even when no refresh is due; a due refresh that cannot be installed, or a launcher still older after refreshing itself once, stops it the same way. Only a release difference triggers a refresh, only launchers that carry the check do it (a box restored from an image made before it still needs `BOA.sh.txt` first), other verbs (`php-idle`, `up-distro`) skip it, and a launcher newer than the tree you asked for is left as it is.
