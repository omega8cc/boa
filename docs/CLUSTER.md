...
> **DEPRECATED / REMOVED (noted 2026-07-04).** The BOA Simple Cluster (Percona XtraDB Cluster + Galera + ProxySQL on Linux-VServer guests) has been discontinued. The `cluster` tool and the Debian-only `mycnfup` tuner it drove on the database nodes have been removed from the BOA tree (a box that still carries `/opt/local/bin/mycnfup` loses it on its next pass), so the install/upgrade recipe below (`wget https://files.boa.io/cluster/cluster.sh`, `cluster in-host`, `cluster in-all`, `cluster in-oct`, etc.) no longer works. BOA's supported redundancy model is now one Percona server per host with DNS-flip failover across two BOA hosts. Only vestigial cluster-marker-gated tooling remains for hosts still carrying the legacy markers (`aegir/tools/system/mysql_cluster_backup.sh`, gated on `/root/.my.cluster_write_node.txt` and `/root/.my.cluster_root_pwd.txt`, and `aegir/tools/system/monitor/check/proxysql.sh`). The OS baseline is also no longer Debian Stretch/jessie: current supported hosts are Devuan Daedalus (recommended) or Debian Bookworm and newer (up to Devuan Excalibur / Debian Trixie). The content below is retained for historical reference only.
