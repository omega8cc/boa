...
Refer to the documentation below, which is also available in every Drupal and Backdrop site's INI template (Grav and Textpattern sites carry no INI and are skipped by the Solr agent). For more information on how to control BOA on site level via INI files, check our [documentation](https://github.com/omega8cc/boa/blob/5.x-dev/docs/ini/site/INI.md).
...
### A core directory Solr does not list
